...
PSQL="psql --username=amjad --dbname=worldcup --no-align --tuples-only -c"
...
echo  "$($PSQL "SELECT DISTINCT(name) FROM teams INNER JOIN games ON teams.team_id=games.winner_id WHERE team_id=winner_id ORDER BY name")" 
